...
First, we'll need to *group* the characters and then *summarize* by counting the *n*umber of characters of each species. 

How do we do that? What functions would we use?
```{r gbsumexercise, exercise = TRUE}
starwars %>%
  
  #we recommend naming the field "count"!
```
...
🙄 Rolling your eyes, you tell them that we'd need to use another function in order to **arrange** this data. To illustrate your point, let's try to **arrange** this data in the section below.
